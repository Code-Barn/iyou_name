    ...
    try:
        ...
        with Image(filename=preview_template_path, resolution=300) as content_img:
            ...
            PRIMARY_BACKGROUND_COLOR = Color(
                user_settings.get("primary_background_color", "#FFFFFF")
            )
            ...
            PRIMARY_BIRTH_PLACE_COLOR = Color(
                user_settings.get("primary_birth_place_color", "black")
            )
            PRIMARY_DEATH_COLOR = Color(
                user_settings.get("primary_death_color", "black")
            )
            PRIMARY_DEATH_PLACE_COLOR = Color(
                user_settings.get("primary_death_place_color", "black")
            )
            PRIMARY_STROKE_COLOR = Color(
                user_settings.get("primary_stroke_color", "black")
            )
            ...
            print(
                f"DEBUG: PRIMARY_BIRTH_PLACE_COLOR set to: {PRIMARY_BIRTH_PLACE_COLOR}"
            )
            print(f"DEBUG: PRIMARY_DEATH_COLOR set to: {PRIMARY_DEATH_COLOR}")
            print(
                f"DEBUG: PRIMARY_DEATH_PLACE_COLOR set to: {PRIMARY_DEATH_PLACE_COLOR}"
            )
            ...
            PRIMARY_BIRTH_TRANSLATE_X = int(
                user_settings.get("primary_birth_translate_x", 0)
            )
            PRIMARY_BIRTH_TRANSLATE_Y = int(
                user_settings.get("primary_birth_translate_y", 0)
            )
            PRIMARY_BIRTH_ROTATE = int(user_settings.get("primary_birth_rotate", -90))
            PRIMARY_BIRTH_PLACE_TRANSLATE_X = int(
                user_settings.get("primary_birth_place_translate_x", 0)
            )
            PRIMARY_BIRTH_PLACE_TRANSLATE_Y = int(
                user_settings.get("primary_birth_place_translate_y", 0)
            )
            PRIMARY_BIRTH_PLACE_ROTATE = int(
                user_settings.get("primary_birth_place_rotate", 0)
            )
            PRIMARY_DEATH_TRANSLATE_X = int(
                user_settings.get("primary_death_translate_x", 0)
            )
            PRIMARY_DEATH_TRANSLATE_Y = int(
                user_settings.get("primary_death_translate_y", 0)
            )
            PRIMARY_DEATH_ROTATE = int(user_settings.get("primary_death_rotate", 0))
            PRIMARY_DEATH_PLACE_TRANSLATE_X = int(
                user_settings.get("primary_death_place_translate_x", 0)
            )
            PRIMARY_DEATH_PLACE_TRANSLATE_Y = int(
                user_settings.get("primary_death_place_translate_y", 0)
            )
            PRIMARY_DEATH_PLACE_ROTATE = int(
                user_settings.get("primary_death_place_rotate", -90)
            )
            ...
            print(
                f"DEBUG: PRIMARY_BIRTH_TRANSLATE_X set to: {PRIMARY_BIRTH_TRANSLATE_X}"
            )
            print(
                f"DEBUG: PRIMARY_BIRTH_TRANSLATE_Y set to: {PRIMARY_BIRTH_TRANSLATE_Y}"
            )
            print(f"DEBUG: PRIMARY_BIRTH_ROTATE set to: {PRIMARY_BIRTH_ROTATE}")
            print(
                f"DEBUG: PRIMARY_BIRTH_PLACE_TRANSLATE_X set to: {PRIMARY_BIRTH_PLACE_TRANSLATE_X}"
            )
            print(
                f"DEBUG: PRIMARY_BIRTH_PLACE_TRANSLATE_Y set to: {PRIMARY_BIRTH_PLACE_TRANSLATE_Y}"
            )
            print(
                f"DEBUG: PRIMARY_BIRTH_PLACE_ROTATE set to: {PRIMARY_BIRTH_PLACE_ROTATE}"
            )
            print(
                f"DEBUG: PRIMARY_DEATH_TRANSLATE_X set to: {PRIMARY_DEATH_TRANSLATE_X}"
            )
            print(
                f"DEBUG: PRIMARY_DEATH_TRANSLATE_Y set to: {PRIMARY_DEATH_TRANSLATE_Y}"
            )
            print(f"DEBUG: PRIMARY_DEATH_ROTATE set to: {PRIMARY_DEATH_ROTATE}")
            print(
                f"DEBUG: PRIMARY_DEATH_PLACE_TRANSLATE_X set to: {PRIMARY_DEATH_PLACE_TRANSLATE_X}"
            )
            print(
                f"DEBUG: PRIMARY_DEATH_PLACE_TRANSLATE_Y set to: {PRIMARY_DEATH_PLACE_TRANSLATE_Y}"
            )
            print(
                f"DEBUG: PRIMARY_DEATH_PLACE_ROTATE set to: {PRIMARY_DEATH_PLACE_ROTATE}"
            )

            # Primary individual font sizes
            PRIMARY_NAME_FONT_SIZE = int(
                user_settings.get("primary_name_font_size", 84)
            )
            PRIMARY_DATE_INFO_FONT_SIZE = int(
                user_settings.get("primary_date_info_font_size", 60)
            )
            PRIMARY_PLACE_INFO_FONT_SIZE = int(
                user_settings.get("primary_place_info_font_size", 28)
            )

            print(f"DEBUG: PRIMARY_NAME_FONT_SIZE set to: {PRIMARY_NAME_FONT_SIZE}")
            print(
                f"DEBUG: PRIMARY_DATE_INFO_FONT_SIZE set to: {PRIMARY_DATE_INFO_FONT_SIZE}"
            )
            print(
                f"DEBUG: PRIMARY_PLACE_INFO_FONT_SIZE set to: {PRIMARY_PLACE_INFO_FONT_SIZE}"
            )
            ...
            with Drawing() as draw:
                ...
                print(
                    f"Drawing rectangle at (left={BACKGROUND_LEFT}, top={BACKGROUND_TOP}, width={BACKGROUND_WIDTH}, height={BACKGROUND_HEIGHT})"
                )
                draw.rectangle(
                ...
                print(
                    f"Translating coordinates by (x={PRIMARY_TRANSLATE_X}, y={PRIMARY_TRANSLATE_Y})"
                )
                ...
                print(
                    f"Drawn text at (0, 0) with primary translation ({PRIMARY_TRANSLATE_X}, {PRIMARY_TRANSLATE_Y}): {display_text}"
                )
                ...
                print(
                    f"Setting font to: {PRIMARY_DATE_INFO_FONT_SIZE} & fill color to: {PRIMARY_BIRTH_COLOR}"
                )
                ...
                print(
                    f"Setting font to: {PRIMARY_PLACE_INFO_FONT_SIZE} & fill color to: {PRIMARY_BIRTH_PLACE_COLOR}"
                )
                ...
                print(
                    f"Setting font to: {PRIMARY_PLACE_INFO_FONT_SIZE} & fill color to: {PRIMARY_DEATH_COLOR}"
                )

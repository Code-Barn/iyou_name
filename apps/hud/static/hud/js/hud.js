...
class FamilyTreeHUD {
  ...
  init() {
    ...
    // Load session data before creating HUD
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("hud-generate")
      .addEventListener("click", (event) => {
        this.generateFinalChart(event);
      });
    ...
    document.getElementById("hud-template").addEventListener("change", (e) => {
      ...
      // Update the hidden template inputs in all forms
      const templateInputs = document.querySelectorAll(
        '[id^="selected-template"], #generate-template',
      );
  ...
  loadSessionData() {
    // Load from Django template
    const individualElement = document.querySelector(
      'input[name="individual_id"]',
    );
    if (individualElement) {
      this.currentIndividual = individualElement.value;
      console.log("Loaded individual from template:", this.currentIndividual);
    }

    const templateElement = document.querySelector('input[name="template"]');
    if (templateElement) {
      this.currentTemplate = templateElement.value;
      console.log("Loaded template from template:", this.currentTemplate);
    }

    const generationsElement = document.querySelector(
      'input[name="generations"]',
    );
    if (generationsElement) {
      this.chartParameters.generations = generationsElement.value;
      console.log(
        "Loaded generations from template:",
        this.chartParameters.generations,
      );
    }
    ...
    if (sessionData.generations) {
      this.chartParameters.generations = sessionData.generations;
    }
  }
  ...
  saveSessionData() {
    const sessionData = {
      ...
      generations: this.chartParameters.generations,
  ...
  async fetchPreviewData() {
    try {
      const response = await fetch(
        `/generator/api/preview/?individual_id=${this.currentIndividual}&template=${this.currentTemplate}&generations=${this.chartParameters.generations}`,
      );
  ...
  drawFamilyTree(data) {
    ...
    if (data.primary) {
      ...
      this.ctx.fillText(
        `Born: ${data.primary.birth_date || "Unknown"}`,
        20,
        50,
      );
    }
  ...
  onGenerationsChange(generations) {
    this.chartParameters.generations = generations;
    this.saveSessionData();
  ...
  resetSettings() {
    ...
    const generationsSelect = document.getElementById("hud-generations");
    if (generationsSelect) {
      generationsSelect.value = "4";
    }
  ...
  generateFinalChart(event) {
    if (!this.currentIndividual) {
      this.showStatus("Please select an individual first", "warning");
      return;
    }

    // Prevent default form submission if this was triggered by an event
    if (event) {
    ...
    // Add individual_id as a hidden input
    ...
    // Add template as a hidden input

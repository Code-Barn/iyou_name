  ...
  function initEventListeners() {
    ...
    if (templateSelect) {
      templateSelect.addEventListener(
        "change",
        HUD.Templates.handleTemplateChange,
      );
    }
  ...
  function saveAndApplySettings() {
    ...
    try {
      ...
      if (HUD.Main.getCurrentTemplate() === "1") {
        ...
        console.log(
          "Stored 1gen settings for future 2gen overlay:",
          userSettings,
        );
      }
...
HUD.Preview = (function () {
  ...
  function generatePreview(userSettings) {
    ...
    // Build request data
    let requestData = {
      individual_id: document.querySelector('input[name="individual_id"]')
        .value,
    ...
    if (currentTemplate === "2") {
      ...
      if (stored1GenSettings) {
        ...
        console.log(
          "Including stored 1gen settings for 2gen overlay:",
          stored1GenSettings,
        );
  ...
  function saveSettings(formData) {
    ...
    fetch("/hud/save-settings/", {
      ...
      .then((response) => {
        if (!response.ok) {
          console.log(
            "Session save failed (non-critical) - settings still work via localStorage",
          );
          return null;
        }
      ...
      .catch((error) => {
        console.log(
          "Session save error (non-critical) - settings still work via localStorage:",
          error.message,
        );
      });
    ...
    return fetch("/hud/get-settings/", {
      ...
      .then((sessionData) => {
        if (sessionData && sessionData.hud_settings) {
          console.log(
            "Loaded settings from session:",
            sessionData.hud_settings,
          );
...
HUD.Templates = (function () {
  ...
  function handleTemplateChange() {
    ...
    const hiddenTemplateInput = document.querySelector(
      'input[name="template"]',
    );
    ...
    loadSettingsPanel(templateValue)
      .then(() => {
      ...
      .then((sessionSettings) => {
        if (sessionSettings) {
          ...
          console.log(
            "No session settings found, trying localStorage for 1gen",
          );
          ...
          if (templateValue === "1") {
            ...
            if (stored1GenSettings) {
              console.log(
                "Updating form with localStorage settings:",
                stored1GenSettings,
              );
  ...
  function loadSettingsPanel(templateValue) {
    ...
    const settingsFile =
      settingsTemplateMap[templateValue] || "default_settings.html";

    // Fetch the appropriate settings template
    return fetch(
      `/hud/get-settings-panel/${settingsFile}/?template=${templateValue}`,
      {
        method: "GET",
        headers: {
          "X-Requested-With": "XMLHttpRequest",
      ...
      .then((html) => {
        ...
        if (templateValue === "2") {
          ...
          if (stored1GenSettings) {
            console.log(
              "Loading stored 1gen settings into 2gen form:",
              stored1GenSettings,
            );
            HUD.Utils.updateFormWithStoredSettings(stored1GenSettings);
          }
          ...
          if (stored1GenSettings) {
            console.log(
              "Loading stored 1gen settings back into 1gen form:",
              stored1GenSettings,
            );
            HUD.Utils.updateFormWithStoredSettings(stored1GenSettings);
          }
  ...
  function updatePreviewImage(templateValue) {
    ...
    if (templateValue === "2") {
      console.log(
        "Template 2 selected - generating preview with stored 1gen settings",
      );
      ...
      if (stored1GenSettings) {
        ...
        console.log(
          "Including stored 1gen settings for 2gen preview:",
          stored1GenSettings,
        );
      ...
      console.log(
        "Template 3 selected - generating preview with 3gen settings",
      );
      ...
      if (stored1GenSettings) {
        userSettings.primary_settings = stored1GenSettings;
        console.log(
          "Including stored 1gen settings for 3gen overlay:",
          stored1GenSettings,
        );
      } else {
      ...
      console.log(
        "Template 4 selected - generating preview with 4gen settings",
      );
      ...
      if (stored1GenSettings) {
        userSettings.primary_settings = stored1GenSettings;
        console.log(
          "Including stored 1gen settings for 4gen overlay:",
          stored1GenSettings,
        );
      } else {
      ...
      const individualId = document.querySelector(
        'input[name="individual_id"]',
      ).value;
      ...
      previewImg.onerror = function () {
        console.error(
          `Failed to load preview for template ${templateValue} with individual_id=${individualId}`,
        );
          ...
          fetch(`/hud/get-file-individuals/?file_id=${fileId}`)
            ...
            .then((data) => {
              if (
                ...
                console.log(
                  `Using fallback individual_id: ${fallbackIndividualId}`,
                );

                // Update the hidden input with the valid ID
                document
                  .querySelectorAll('input[name="individual_id"]')
                  .forEach((input) => {
...
HUD.Sliders = (function () {
  ...
  function initializeAll() {
    ...
    // Font size sliders
    setupSlider(
      "primary-name-font-size-slider",
      "primary-name-font-size-value",
    );
    setupSlider(
      "primary-date-info-font-size-slider",
      "primary-date-info-font-size-value",
    );
    setupSlider(
      "primary-place-info-font-size-slider",
      "primary-place-info-font-size-value",
    );
    ...
    // Birth date sliders
    setupSlider(
      "primary-birth-translate-x-slider",
      "primary-birth-translate-x-value",
    );
    setupSlider(
      "primary-birth-translate-y-slider",
      "primary-birth-translate-y-value",
    );
    setupSlider("primary-birth-rotate-slider", "primary-birth-rotate-value");

    // Birth place sliders
    setupSlider(
      "primary-birth-place-translate-x-slider",
      "primary-birth-place-translate-x-value",
    );
    setupSlider(
      "primary-birth-place-translate-y-slider",
      "primary-birth-place-translate-y-value",
    );
    setupSlider(
      "primary-birth-place-rotate-slider",
      "primary-birth-place-rotate-value",
    );

    // Death date sliders
    setupSlider(
      "primary-death-translate-x-slider",
      "primary-death-translate-x-value",
    );
    setupSlider(
      "primary-death-translate-y-slider",
      "primary-death-translate-y-value",
    );
    setupSlider("primary-death-rotate-slider", "primary-death-rotate-value");

    // Death place sliders
    setupSlider(
      "primary-death-place-translate-x-slider",
      "primary-death-place-translate-x-value",
    );
    setupSlider(
      "primary-death-place-translate-y-slider",
      "primary-death-place-translate-y-value",
    );
    setupSlider(
      "primary-death-place-rotate-slider",
      "primary-death-place-rotate-value",
    );

    // 2gen specific sliders (if present)
    setupSlider(
      "parent-father-name-font-size-slider",
      "parent-father-name-font-size-value",
    );
    setupSlider(
      "parent-mother-name-font-size-slider",
      "parent-mother-name-font-size-value",
    );
    setupSlider(
      "parent-date-info-font-size-slider",
      "parent-date-info-font-size-value",
    );
    setupSlider(
      "parent-place-info-font-size-slider",
      "parent-place-info-font-size-value",
    );
...
HUD.Rotation = (function () {
  ...
  function updateRotation() {
    ...
    console.log(
      `Rotated preview to ${currentRotation}° (normalized: ${normalizedRotation}°)`,
    );
  }
...
HUD.Utils = (function () {
  ...
  function updateFinalChartForm(userSettings) {
    try {
      ...
      let finalChartForm = document.querySelector(
        'form[action*="generate_final_chart"]',
      );
      if (!finalChartForm) {
      ...
      if (finalChartForm) {
        ...
        console.log(
          `Successfully updated ${updatedCount} settings in final chart form`,
        );
      } else {
